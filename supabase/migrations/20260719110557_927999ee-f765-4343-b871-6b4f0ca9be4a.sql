...
BEGIN
  LOOP
    ...
    SET LOCAL lock_timeout = '5s';
      EXECUTE 'ALTER TABLE public.guidance_records ENABLE ROW LEVEL SECURITY';
    EXIT;
    EXCEPTION
...
BEGIN
  LOOP
    BEGIN
    SET LOCAL lock_timeout = '5s';
      EXECUTE 'DROP POLICY IF EXISTS school_scope_restrictive ON public.guidance_records';
    EXIT;
    EXCEPTION
      WHEN deadlock_detected OR lock_not_available THEN
        _ddl_try := _ddl_try + 1;
...
BEGIN
  LOOP
    BEGIN
    SET LOCAL lock_timeout = '5s';
      EXECUTE 'CREATE POLICY school_scope_restrictive ON public.guidance_records
...
BEGIN
  LOOP
    BEGIN
    SET LOCAL lock_timeout = '5s';
      EXECUTE 'DROP POLICY IF EXISTS school_scope_teacher ON public.guidance_records';
    EXIT;
    EXCEPTION
      WHEN deadlock_detected OR lock_not_available THEN
        _ddl_try := _ddl_try + 1;
...
BEGIN
  LOOP
    BEGIN
    SET LOCAL lock_timeout = '5s';
      EXECUTE 'CREATE POLICY school_scope_teacher ON public.guidance_records
      AS RESTRICTIVE FOR ALL TO authenticated
      USING (
...
BEGIN
  LOOP
    BEGIN
    SET LOCAL lock_timeout = '5s';
      EXECUTE 'DROP POLICY IF EXISTS "Student visibility scoped" ON public.students';
    EXIT;
    EXCEPTION
      WHEN deadlock_detected OR lock_not_available THEN
        _ddl_try := _ddl_try + 1;
...
BEGIN
  LOOP
    BEGIN
    SET LOCAL lock_timeout = '5s';
      EXECUTE 'CREATE POLICY "Student visibility scoped" ON public.students
...
BEGIN
  LOOP
    BEGIN
    SET LOCAL lock_timeout = '5s';
      EXECUTE 'DROP POLICY IF EXISTS "Observers can view" ON public.students';
    EXIT;
    EXCEPTION
      WHEN deadlock_detected OR lock_not_available THEN
        _ddl_try := _ddl_try + 1;
...
BEGIN
  LOOP
    BEGIN
    SET LOCAL lock_timeout = '5s';
      EXECUTE 'CREATE POLICY "Observers can view" ON public.students
      FOR SELECT TO authenticated
...
BEGIN
  LOOP
    BEGIN
    SET LOCAL lock_timeout = '5s';
      EXECUTE 'DROP POLICY IF EXISTS "Observers can view" ON public.attendance';
    EXIT;
    EXCEPTION
      WHEN deadlock_detected OR lock_not_available THEN
        _ddl_try := _ddl_try + 1;
...
BEGIN
  LOOP
    BEGIN
    SET LOCAL lock_timeout = '5s';
      EXECUTE 'CREATE POLICY "Observers can view" ON public.attendance
      FOR SELECT TO authenticated
      USING ((SELECT has_role(auth.uid(), ''observer''::app_role)))';
    EXIT;
    EXCEPTION
...
BEGIN
  LOOP
    BEGIN
    SET LOCAL lock_timeout = '5s';
      EXECUTE 'DROP POLICY IF EXISTS "Observers can view" ON public.enrollments';
    EXIT;
    EXCEPTION
      WHEN deadlock_detected OR lock_not_available THEN
        _ddl_try := _ddl_try + 1;
...
BEGIN
  LOOP
    BEGIN
    SET LOCAL lock_timeout = '5s';
      EXECUTE 'CREATE POLICY "Observers can view" ON public.enrollments
      FOR SELECT TO authenticated
      USING ((SELECT has_role(auth.uid(), ''observer''::app_role)))';
    EXIT;
    EXCEPTION
...
BEGIN
  LOOP
    BEGIN
    SET LOCAL lock_timeout = '5s';
      EXECUTE 'DROP POLICY IF EXISTS "Staff manage enrollments" ON public.enrollments';
    EXIT;
    EXCEPTION
      WHEN deadlock_detected OR lock_not_available THEN
        _ddl_try := _ddl_try + 1;
...
BEGIN
  LOOP
    BEGIN
    SET LOCAL lock_timeout = '5s';
      EXECUTE 'CREATE POLICY "Staff manage enrollments" ON public.enrollments
...
BEGIN
  LOOP
    BEGIN
    SET LOCAL lock_timeout = '5s';
      EXECUTE 'DROP POLICY IF EXISTS "Staff view all enrollments" ON public.enrollments';
    EXIT;
    EXCEPTION
      WHEN deadlock_detected OR lock_not_available THEN
        _ddl_try := _ddl_try + 1;
...
BEGIN
  LOOP
    BEGIN
    SET LOCAL lock_timeout = '5s';
      EXECUTE 'CREATE POLICY "Staff view all enrollments" ON public.enrollments
      FOR SELECT TO authenticated
      USING ((SELECT (has_role(auth.uid(), ''admin''::app_role) OR has_role(auth.uid(), ''director''::app_role) OR has_role(auth.uid(), ''teacher''::app_role))))';
    EXIT;
    EXCEPTION
...
BEGIN
  LOOP
    BEGIN
    SET LOCAL lock_timeout = '5s';
      EXECUTE 'CREATE POLICY "Students view own enrollments" ON public.enrollments
      FOR SELECT TO authenticated
      USING (student_id IN (SELECT id FROM public.students WHERE auth_user_id = (SELECT auth.uid())))';
    EXIT;
    EXCEPTION
