...
3. A list of question rows, each with exactly 4 bubbles: A, B, C, D (no E exists on this sheet)
...
- For each question row: choose the ONE letter whose bubble is darkest. If none or more than one is clearly filled, choose null.
...
Use null when no bubble (or multiple bubbles) is clearly marked. Do not emit letters outside A-D.`;
...
Detect the student code and the answer (A/B/C/D) for each question. Return JSON only.`;
    ...
    // answers must be object keyed by question number; values must be A-D or null
    const rawAnswers = parsed.answers;
    const answers: Record<string, string | null> = {};
    if (rawAnswers && typeof rawAnswers === "object" && !Array.isArray(rawAnswers)) {
      for (const [k, v] of Object.entries(rawAnswers)) {
        const letter = String(v ?? "").trim().toUpperCase();
        if (!VALID_LETTERS.includes(letter)) continue; // drop invalid (incl. "E")
