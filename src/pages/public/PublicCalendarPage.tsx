  ...
  useEffect(() => {
    (async () => {
      const { data } = await supabase
        ...
        .gte("event_date", new Date(Date.now() - 30 * 24 * 3600 * 1000).toISOString().slice(0, 10))
            ...
            <div key={e.id} className="flex gap-4 rounded-2xl border border-border/50 bg-background/70 p-4 shadow-sm backdrop-blur">
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="mt-1 flex flex-wrap items-center gap-3 text-xs text-muted-foreground">
                  <span className="inline-flex items-center gap-1"><CalIcon className="h-3.5 w-3.5" />{d.toLocaleDateString("th-TH", { day: "numeric", month: "long", year: "numeric" })}</span>
