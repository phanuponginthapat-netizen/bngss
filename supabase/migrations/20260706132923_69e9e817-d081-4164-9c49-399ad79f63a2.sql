...
BEGIN
  LOOP
    BEGIN
    SET LOCAL lock_timeout = '5s';
      EXECUTE 'DROP POLICY IF EXISTS "Students view own behavior" ON public.behavior_records';
    EXIT;
    EXCEPTION
      WHEN deadlock_detected OR lock_not_available THEN
        _ddl_try := _ddl_try + 1;
...
BEGIN
  LOOP
    BEGIN
    SET LOCAL lock_timeout = '5s';
      EXECUTE 'CREATE POLICY "Parents view child behavior" ON public.behavior_records
...
BEGIN
  LOOP
    BEGIN
    SET LOCAL lock_timeout = '5s';
      EXECUTE 'CREATE POLICY "Staff can manage behavior_records" ON public.behavior_records
    FOR ALL
...
BEGIN
  LOOP
    BEGIN
    SET LOCAL lock_timeout = '5s';
      EXECUTE 'CREATE POLICY "Students view own behavior" ON public.behavior_records
...
BEGIN
  LOOP
    BEGIN
    SET LOCAL lock_timeout = '5s';
      EXECUTE 'CREATE POLICY "Staff can manage health_records" ON public.health_records
    FOR ALL
    USING ((SELECT has_role(auth.uid(), ''admin''::app_role) OR has_role(auth.uid(), ''director''::app_role) OR has_role(auth.uid(), ''teacher''::app_role)))
    WITH CHECK ((SELECT has_role(auth.uid(), ''admin''::app_role) OR has_role(auth.uid(), ''director''::app_role) OR has_role(auth.uid(), ''teacher''::app_role)))';
    EXIT;
...
BEGIN
  LOOP
    BEGIN
    SET LOCAL lock_timeout = '5s';
      EXECUTE 'CREATE POLICY "Students can view their own health records" ON public.health_records
    FOR SELECT USING (student_id IN (SELECT id FROM students WHERE auth_user_id = (SELECT auth.uid())))';
    EXIT;
    EXCEPTION
      WHEN deadlock_detected OR lock_not_available THEN
...
BEGIN
  LOOP
    BEGIN
    SET LOCAL lock_timeout = '5s';
      EXECUTE 'CREATE POLICY "Staff can manage sdq_records" ON public.sdq_records
    FOR ALL
    USING ((SELECT has_role(auth.uid(), ''admin''::app_role) OR has_role(auth.uid(), ''director''::app_role) OR has_role(auth.uid(), ''teacher''::app_role)))
    WITH CHECK ((SELECT has_role(auth.uid(), ''admin''::app_role) OR has_role(auth.uid(), ''director''::app_role) OR has_role(auth.uid(), ''teacher''::app_role)))';
    EXIT;
...
BEGIN
  LOOP
    BEGIN
    SET LOCAL lock_timeout = '5s';
      EXECUTE 'CREATE POLICY "Students can view their own sdq records" ON public.sdq_records
    FOR SELECT USING (student_id IN (SELECT id FROM students WHERE auth_user_id = (SELECT auth.uid())))';
    EXIT;
    EXCEPTION
      WHEN deadlock_detected OR lock_not_available THEN
...
BEGIN
  LOOP
    BEGIN
    SET LOCAL lock_timeout = '5s';
      EXECUTE 'CREATE POLICY "Admin director manage student_leaves" ON public.student_leaves
    FOR ALL
...
BEGIN
  LOOP
    BEGIN
    SET LOCAL lock_timeout = '5s';
      EXECUTE 'CREATE POLICY "Homeroom teacher manage student_leaves" ON public.student_leaves
    FOR ALL
...
BEGIN
  LOOP
    BEGIN
    SET LOCAL lock_timeout = '5s';
      EXECUTE 'CREATE POLICY "Parents request child leaves" ON public.student_leaves
...
BEGIN
  LOOP
    BEGIN
    SET LOCAL lock_timeout = '5s';
      EXECUTE 'CREATE POLICY "Parents view child leaves" ON public.student_leaves
    FOR SELECT USING ((SELECT is_parent_of(auth.uid(), student_id)))';
    EXIT;
    EXCEPTION
      WHEN deadlock_detected OR lock_not_available THEN
...
BEGIN
  LOOP
    BEGIN
    SET LOCAL lock_timeout = '5s';
      EXECUTE 'CREATE POLICY "Students can request their own leaves" ON public.student_leaves
...
BEGIN
  LOOP
    BEGIN
    SET LOCAL lock_timeout = '5s';
      EXECUTE 'CREATE POLICY "Students can view their own leaves" ON public.student_leaves
    FOR SELECT USING (student_id IN (SELECT id FROM students WHERE auth_user_id = (SELECT auth.uid())))';
    EXIT;
    EXCEPTION
      WHEN deadlock_detected OR lock_not_available THEN
...
BEGIN
  LOOP
    BEGIN
    SET LOCAL lock_timeout = '5s';
      EXECUTE 'CREATE POLICY "admins manage all submissions" ON public.homework_submissions
    FOR ALL
    USING ((SELECT has_role(auth.uid(), ''admin''::app_role) OR has_role(auth.uid(), ''director''::app_role)))
    WITH CHECK ((SELECT has_role(auth.uid(), ''admin''::app_role) OR has_role(auth.uid(), ''director''::app_role)))';
    EXIT;
...
BEGIN
  LOOP
    BEGIN
    SET LOCAL lock_timeout = '5s';
      EXECUTE 'CREATE POLICY "assignment owner can grade submissions" ON public.homework_submissions
    FOR UPDATE USING (
      EXISTS (
...
BEGIN
  LOOP
    BEGIN
    SET LOCAL lock_timeout = '5s';
      EXECUTE 'CREATE POLICY "students manage own submissions" ON public.homework_submissions
    FOR ALL
...
BEGIN
  LOOP
    BEGIN
    SET LOCAL lock_timeout = '5s';
      EXECUTE 'DROP POLICY IF EXISTS "Users delete own notifications" ON public.notifications';
    EXIT;
    EXCEPTION
      WHEN deadlock_detected OR lock_not_available THEN
        _ddl_try := _ddl_try + 1;
...
BEGIN
  LOOP
    BEGIN
    SET LOCAL lock_timeout = '5s';
      EXECUTE 'CREATE POLICY "Users can view own notifications" ON public.notifications
...
BEGIN
  LOOP
    BEGIN
    SET LOCAL lock_timeout = '5s';
      EXECUTE 'CREATE POLICY "Users delete own notifications" ON public.notifications
...
BEGIN
  LOOP
    BEGIN
    SET LOCAL lock_timeout = '5s';
      EXECUTE 'CREATE POLICY "Users insert own notifications or admin insert any" ON public.notifications
