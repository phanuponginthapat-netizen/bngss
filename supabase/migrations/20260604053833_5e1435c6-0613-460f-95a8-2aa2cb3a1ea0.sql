...
BEGIN
  LOOP
    BEGIN
    SET LOCAL lock_timeout = '5s';
      EXECUTE 'CREATE POLICY "exam-scans authenticated read" ON storage.objects FOR SELECT TO authenticated
...
BEGIN
  LOOP
    BEGIN
    SET LOCAL lock_timeout = '5s';
      EXECUTE 'CREATE POLICY "exam-scans authenticated insert" ON storage.objects FOR INSERT TO authenticated
...
BEGIN
  LOOP
    BEGIN
    SET LOCAL lock_timeout = '5s';
      EXECUTE 'CREATE POLICY "exam-scans authenticated update" ON storage.objects FOR UPDATE TO authenticated
      USING (bucket_id = ''exam-scans'')';
    EXIT;
    EXCEPTION
      WHEN deadlock_detected OR lock_not_available THEN
...
BEGIN
  LOOP
    BEGIN
    SET LOCAL lock_timeout = '5s';
      EXECUTE 'CREATE POLICY "exam-scans authenticated delete" ON storage.objects FOR DELETE TO authenticated
      USING (bucket_id = ''exam-scans'')';
    EXIT;
    EXCEPTION
      WHEN deadlock_detected OR lock_not_available THEN
