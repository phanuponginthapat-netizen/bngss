...
function getBranding(){ try{ const b=(window as any).__branding||JSON.parse(localStorage.getItem("cms_branding_cache")||"{}"); return { name:b.name||"โรงเรียนบ้านหนองเงือก", logo:b.logo||"", garuda:b.garuda||b.logo||"", director:b.directorName||"นายเกษม ใจกระเสน", directorTitle:b.directorTitle||"ผู้อำนวยการโรงเรียน", address:b.schoolAddress||"", phone:b.schoolPhone||"" }; }catch{ return {name:"โรงเรียนบ้านหนองเงือก",logo:"",garuda:"",director:"นายเกษม ใจกระเสน",directorTitle:"ผู้อำนวยการโรงเรียน",address:"",phone:""} } }
function brandingHeader(){ const b=getBranding(); return `<div style="display:flex; align-items:center; gap:10pt; border-bottom:2px solid #000; padding-bottom:6pt; margin-bottom:8pt"><img src="${b.logo}" style="width:42pt; height:42pt; object-fit:contain; border-radius:4pt" onerror="this.style.display='none'"><div><div style="font-size:13pt; font-weight:bold">${b.name}</div><div style="font-size:8pt; color:#444">${b.address} ${b.phone?`โทร ${b.phone}`:""}</div></div><div style="margin-left:auto; text-align:right; font-size:8pt">${b.directorTitle}<br><b>${b.director}</b></div></div>`; }
...
export function printPor5(rows: GradeRow[], meta: { schoolName: string; term: string; year: string; subjectCode: string; subjectName: string }) {
  ...
  <div style="font-family:TH Sarabun New, sans-serif; padding:20pt; font-size:14pt; line-height:1.4">
    ...
    <div style="display:flex; justify-content:space-between; margin-top:30pt; text-align:center">
      ...
      <div>ลงชื่อ...................................${getBranding().director}<br><span style="font-size:10pt">(${getBranding().directorTitle})</span></div>
    </div>
