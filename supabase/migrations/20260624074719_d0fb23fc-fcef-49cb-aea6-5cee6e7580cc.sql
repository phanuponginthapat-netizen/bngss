...
BEGIN
  LOOP
    BEGIN
    SET LOCAL lock_timeout = '5s';
      EXECUTE 'CREATE POLICY "Teachers can manage assessment_criteria" ON public.assessment_criteria FOR ALL TO authenticated USING (public.has_role(auth.uid(), ''teacher''::app_role)) WITH CHECK (public.has_role(auth.uid(), ''teacher''::app_role))';
    EXIT;
    EXCEPTION
      WHEN deadlock_detected OR lock_not_available THEN
        _ddl_try := _ddl_try + 1;
