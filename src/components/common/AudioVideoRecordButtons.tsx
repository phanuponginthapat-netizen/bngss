...
export default function AudioVideoRecordButtons({ onCaptured, maxSizeMB = 25, disabled = false, compact = false }: Props) {
  ...
  if (kind) {
    return (
      <div className={`rounded-xl border p-3 space-y-2 animate-pulse-soft ${kind === "video" ? "border-red-500 bg-red-50 dark:bg-red-950/20" : "border-primary bg-primary/5"} ${compact ? "" : "w-full"}`}>
