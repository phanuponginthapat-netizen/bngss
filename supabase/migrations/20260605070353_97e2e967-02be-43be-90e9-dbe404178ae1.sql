...
AS $$
  SELECT public.can_access_eform_attachment(
    NULLIF((storage.foldername(_object_name))[1], '')::uuid,
    _user_id
  )
  AND EXISTS (
    SELECT 1
    FROM public.eforms e
    WHERE e.id = NULLIF((storage.foldername(_object_name))[1], '')::uuid
