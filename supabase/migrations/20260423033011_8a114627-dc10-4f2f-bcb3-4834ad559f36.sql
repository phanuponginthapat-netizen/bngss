...
DO $guard$
BEGIN
  EXECUTE 'CREATE POLICY "Staff can manage students"
  ON public.students
  FOR ALL
  TO authenticated
  USING (
...
DO $guard$
BEGIN
  EXECUTE 'CREATE POLICY "Students can view their own record"
  ON public.students
  FOR SELECT
  TO authenticated
...
DO $guard$
BEGIN
  EXECUTE 'CREATE POLICY "Staff can manage student_subsidies"
...
DO $guard$
BEGIN
  EXECUTE 'CREATE POLICY "Parents can view linked student subsidies"
  ON public.student_subsidies
  FOR SELECT
  TO authenticated
  USING (
