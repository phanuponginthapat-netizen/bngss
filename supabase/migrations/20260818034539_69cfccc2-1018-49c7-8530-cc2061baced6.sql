CREATE OR REPLACE FUNCTION public.ensure_attendance_digest_cron()
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  LOOP
    BEGIN
      ...
      EXECUTE 'REVOKE ALL ON FUNCTION public.ensure_attendance_digest_cron() FROM PUBLIC';
    ...
    EXCEPTION
      ...
      WHEN insufficient_privilege OR undefined_function OR undefined_object THEN
        RAISE NOTICE 'skipped grant: %', SQLERRM;
        EXIT;
    END;
  END LOOP;
END
