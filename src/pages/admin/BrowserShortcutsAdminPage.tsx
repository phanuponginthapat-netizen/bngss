...
import { Globe, Plus, Trash2, GripVertical, X } from "lucide-react";
...
const COMMON_ICONS = [
  "FileText", "Sheet", "Presentation", "HardDrive", "Mail", "GraduationCap",
  "Youtube", "Languages", "Map", "BookOpen", "Globe", "Search", "Newspaper",
  "Calculator", "Music", "Image", "Video", "Code", "Chrome", "Github", "Figma",
];

function IconPreview({ name, className }: { name: string | null; className?: string }) {
  const Comp = (name && (LucideIcons as any)[name]) || Globe;
...
export default function BrowserShortcutsAdminPage() {
  ...
  const { data: items = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      return (data as unknown as Shortcut[]) || [];
    },
  });

  const invalidate = () => qc.invalidateQueries({ queryKey: ["browser_shortcuts"] });
  ...
  const move = async (s: Shortcut, dir: -1 | 1) => {
    ...
    await supabase.from("browser_shortcuts" as any).update({ sort_order: b.sort_order }).eq("id", a.id);
    await supabase.from("browser_shortcuts" as any).update({ sort_order: a.sort_order }).eq("id", b.id);
    invalidate();
  };

  const uploadLogo = async (file: File) => {
    if (!editing) return;
    if (!file.type.startsWith("image/")) return toast.error(L("ต้องเป็นรูปภาพ", "Must be image"));
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-2 flex-wrap">
        <div>
          <h1 className="text-2xl font-bold flex items-center gap-2"><Globe className="w-6 h-6" /> {L("ปุ่มลัด Safe Browser", "Safe Browser Shortcuts")}</h1>
          <p className="text-sm text-muted-foreground">{L("จัดการปุ่มลัดที่แสดงในหน้า Safe Browser", "Manage shortcut tiles on Safe Browser page")}</p>
        </div>
        <Button onClick={() => setEditing({ ...empty, sort_order: (items.at(-1)?.sort_order ?? 0) + 10 })}>
          <Plus className="w-4 h-4 mr-1" /> {L("เพิ่มปุ่ม", "Add")}
        </Button>
      </div>

      <Card>
        <CardHeader><CardTitle>{L("รายการ", "Shortcuts")} ({items.length})</CardTitle></CardHeader>
        <CardContent>
          {isLoading ? <div className="text-sm text-muted-foreground">Loading…</div>
            : items.length === 0 ? <div className="text-sm text-muted-foreground">{L("ยังไม่มี", "No items")}</div>
            : (
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
                {items.map((s) => (
                  <div key={s.id} className={`flex items-center gap-3 rounded-xl border p-3 ${s.is_active ? "" : "opacity-60"}`}>
                    <div className="flex flex-col">
                      <button onClick={() => move(s, -1)} className="text-muted-foreground text-xs">▲</button>
                    ...
                    <div className={`w-12 h-12 rounded-2xl ${s.bg_class} flex items-center justify-center overflow-hidden ring-1 ring-black/5 shrink-0`}>
                      {s.logo_url ? <img loading="lazy" decoding="async" src={s.logo_url} alt="" className="w-full h-full object-contain p-1.5" /> : <IconPreview name={s.icon} className="w-6 h-6 text-white" />}
                    </div>
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="flex flex-wrap gap-1 mt-1">
                        {s.visible_roles.map((r) => <Badge key={r} variant="secondary" className="text-[10px] py-0 px-1.5">{r}</Badge>)}
                      </div>
                    </div>
                    <div className="flex flex-col items-end gap-1">
                      <Switch checked={s.is_active} onCheckedChange={() => toggleActive(s)} />
                      <div className="flex gap-1">
                        <Button size="sm" variant="outline" onClick={() => setEditing(s)}>{L("แก้ไข", "Edit")}</Button>
      ...
      <Dialog open={!!editing} onOpenChange={(o) => !o && setEditing(null)}>
        <DialogContent className="sm:max-w-2xl sm:max-h-[90vh] overflow-y-auto">
          <DialogHeader><DialogTitle>{editing?.id ? L("แก้ไข", "Edit") : L("เพิ่มปุ่ม", "Add Shortcut")}</DialogTitle></DialogHeader>
          {editing && (
            <div className="space-y-4">
              <div className="flex items-center gap-4 p-4 rounded-xl bg-muted/30">
                <div className={`w-16 h-16 rounded-2xl ${editing.bg_class} flex items-center justify-center overflow-hidden ring-1 ring-black/5`}>
                  {editing.logo_url ? <img loading="lazy" decoding="async" src={editing.logo_url} alt="" className="w-full h-full object-contain p-2" /> : <IconPreview name={editing.icon} className="w-8 h-8 text-white" />}
                </div>
                <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div><Label>{L("ชื่อ (ไทย)", "Label (TH)")}</Label><Input value={editing.label_th} onChange={(e) => setEditing({ ...editing, label_th: e.target.value })} /></div>
                <div><Label>{L("ชื่อ (อังกฤษ)", "Label (EN)")}</Label><Input value={editing.label_en} onChange={(e) => setEditing({ ...editing, label_en: e.target.value })} /></div>
              </div>

              <div>
                <Label>{L("URL ปลายทาง", "Target URL")}</Label>
                <Input placeholder="https://..." value={editing.target_url} onChange={(e) => setEditing({ ...editing, target_url: e.target.value })} />
              </div>

              <div>
                <Label>{L("ไอคอน (Lucide)", "Icon (Lucide)")}</Label>
                <div className="flex gap-2 items-center">
                  <Input placeholder="Globe, Youtube, Mail..." value={editing.icon || ""} onChange={(e) => setEditing({ ...editing, icon: e.target.value })} />
                ...
                <div className="flex flex-wrap gap-1 mt-2">
                  {COMMON_ICONS.map((n) => (
                    <button key={n} type="button" onClick={() => setEditing({ ...editing, icon: n })}
                      className={`p-1.5 rounded border hover:bg-muted ${editing.icon === n ? "bg-primary/10 border-primary" : ""}`} title={n}>
              ...
              <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-5 gap-2 mt-1">
                  {BG_PRESETS.map((bg) => (
                    <button key={bg} type="button" onClick={() => setEditing({ ...editing, bg_class: bg })}
              ...
              <div>
                <Label>{L("บทบาทที่เห็น", "Visible to roles")}</Label>
                <div className="flex flex-wrap gap-2 mt-1">
                  {ROLES.map((r) => {
                    const on = editing.visible_roles.includes(r);
                    return (
                      <button key={r} type="button"
                        onClick={() => setEditing({ ...editing, visible_roles: on ? editing.visible_roles.filter((x) => x !== r) : [...editing.visible_roles, r] })}
                        className={`px-3 py-1 rounded-full text-xs border ${on ? "bg-primary text-primary-foreground border-primary" : "bg-background"}`}>{r}</button>
                    );
                  })}
                </div>
              </div>
