-- ทำให้ฟังก์ชันลงทะเบียนใบหน้าด้วยตนเองพร้อมใช้งานและมีสิทธิ์ครบ (idempotent)
...
do $$
begin
  execute 'grant execute on function public.self_enroll_personnel_face(jsonb) to authenticated, service_role';
  execute 'grant execute on function public.self_enroll_face(jsonb, text[], text) to authenticated, service_role';
  execute 'grant execute on function public.check_face_duplicate(uuid, jsonb, real) to authenticated, service_role';
  execute 'grant execute on function public.face_distance(real[], real[]) to anon, authenticated, service_role';
