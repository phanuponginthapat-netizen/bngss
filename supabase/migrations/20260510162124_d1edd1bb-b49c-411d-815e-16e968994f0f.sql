...
BEGIN
  LOOP
    ...
    SET LOCAL lock_timeout = '5s';
      EXECUTE 'DROP POLICY IF EXISTS "Authenticated users can view iot devices" ON public.iot_devices';
    EXIT;
    EXCEPTION
...
BEGIN
  LOOP
    BEGIN
    SET LOCAL lock_timeout = '5s';
      EXECUTE 'CREATE POLICY "Staff can view iot devices" ON public.iot_devices
...
BEGIN
  LOOP
    BEGIN
    SET LOCAL lock_timeout = '5s';
      EXECUTE 'DROP POLICY IF EXISTS "Staff evaluations restricted view" ON public.staff_evaluations';
    EXIT;
    EXCEPTION
      WHEN deadlock_detected OR lock_not_available THEN
        _ddl_try := _ddl_try + 1;
...
BEGIN
  LOOP
    BEGIN
    SET LOCAL lock_timeout = '5s';
      EXECUTE 'CREATE POLICY "Staff evaluations restricted view" ON public.staff_evaluations
      FOR SELECT TO authenticated
      USING (
...
BEGIN
  LOOP
    BEGIN
    SET LOCAL lock_timeout = '5s';
      EXECUTE 'DROP POLICY IF EXISTS "PA agreements restricted view" ON public.pa_agreements';
    EXIT;
    EXCEPTION
      WHEN deadlock_detected OR lock_not_available THEN
        _ddl_try := _ddl_try + 1;
...
BEGIN
  LOOP
    BEGIN
    SET LOCAL lock_timeout = '5s';
      EXECUTE 'CREATE POLICY "PA agreements restricted view" ON public.pa_agreements
      FOR SELECT TO authenticated
      USING (
        has_role(auth.uid(),''admin'') OR has_role(auth.uid(),''director'')
...
BEGIN
  LOOP
    BEGIN
    SET LOCAL lock_timeout = '5s';
      EXECUTE 'DROP POLICY IF EXISTS "PA indicator scores restricted view" ON public.pa_indicator_scores';
    EXIT;
    EXCEPTION
      WHEN deadlock_detected OR lock_not_available THEN
        _ddl_try := _ddl_try + 1;
...
BEGIN
  LOOP
    BEGIN
    SET LOCAL lock_timeout = '5s';
      EXECUTE 'CREATE POLICY "PA indicator scores restricted view" ON public.pa_indicator_scores
      FOR SELECT TO authenticated
      USING (
        has_role(auth.uid(),''admin'') OR has_role(auth.uid(),''director'')
        OR EXISTS (
...
BEGIN
  LOOP
    BEGIN
    SET LOCAL lock_timeout = '5s';
      EXECUTE 'DROP POLICY IF EXISTS "Staff can view homeroom_records" ON public.homeroom_records';
    EXIT;
    EXCEPTION
      WHEN deadlock_detected OR lock_not_available THEN
        _ddl_try := _ddl_try + 1;
...
BEGIN
  LOOP
    BEGIN
    SET LOCAL lock_timeout = '5s';
      EXECUTE 'CREATE POLICY "Staff can view homeroom_records" ON public.homeroom_records
      FOR SELECT TO authenticated
      USING (has_role(auth.uid(),''admin'') OR has_role(auth.uid(),''director'') OR has_role(auth.uid(),''teacher''))';
    EXIT;
    EXCEPTION
...
BEGIN
  LOOP
    BEGIN
    SET LOCAL lock_timeout = '5s';
      EXECUTE 'DROP POLICY IF EXISTS "Admin/Director view account_balances" ON public.account_balances';
    EXIT;
    EXCEPTION
      WHEN deadlock_detected OR lock_not_available THEN
        _ddl_try := _ddl_try + 1;
...
BEGIN
  LOOP
    BEGIN
    SET LOCAL lock_timeout = '5s';
      EXECUTE 'CREATE POLICY "Admin/Director view account_balances" ON public.account_balances
      FOR SELECT TO authenticated
...
BEGIN
  LOOP
    BEGIN
    SET LOCAL lock_timeout = '5s';
      EXECUTE 'DROP POLICY IF EXISTS "Admin/Director view procurement_records" ON public.procurement_records';
    EXIT;
    EXCEPTION
      WHEN deadlock_detected OR lock_not_available THEN
        _ddl_try := _ddl_try + 1;
...
BEGIN
  LOOP
    BEGIN
    SET LOCAL lock_timeout = '5s';
      EXECUTE 'CREATE POLICY "Admin/Director view procurement_records" ON public.procurement_records
      FOR SELECT TO authenticated
      USING (has_role(auth.uid(),''admin'') OR has_role(auth.uid(),''director''))';
    EXIT;
    EXCEPTION
...
BEGIN
  LOOP
    BEGIN
    SET LOCAL lock_timeout = '5s';
      EXECUTE 'DROP POLICY IF EXISTS "ID plan records restricted view" ON public.id_plan_records';
    EXIT;
    EXCEPTION
      WHEN deadlock_detected OR lock_not_available THEN
        _ddl_try := _ddl_try + 1;
...
BEGIN
  LOOP
    BEGIN
    SET LOCAL lock_timeout = '5s';
      EXECUTE 'CREATE POLICY "ID plan records restricted view" ON public.id_plan_records
      FOR SELECT TO authenticated
      USING (
        has_role(auth.uid(),''admin'') OR has_role(auth.uid(),''director'')
...
BEGIN
  LOOP
    BEGIN
    SET LOCAL lock_timeout = '5s';
      EXECUTE 'DROP POLICY IF EXISTS "Staff can view attendance photos" ON storage.objects';
    EXIT;
    EXCEPTION
      WHEN deadlock_detected OR lock_not_available THEN
        _ddl_try := _ddl_try + 1;
...
BEGIN
  LOOP
    BEGIN
    SET LOCAL lock_timeout = '5s';
      EXECUTE 'CREATE POLICY "Staff can view attendance photos" ON storage.objects
      FOR SELECT TO authenticated
      USING (
...
BEGIN
  LOOP
    BEGIN
    SET LOCAL lock_timeout = '5s';
      EXECUTE 'CREATE POLICY "eform attach: sender or recipient can view" ON storage.objects
      FOR SELECT TO authenticated
      USING (
