...
BEGIN
  LOOP
    ...
    SET LOCAL lock_timeout = '5s';
      EXECUTE 'REVOKE SELECT (api_key) ON public.ai_providers FROM authenticated, anon';
    EXIT;
    EXCEPTION
...
BEGIN
  LOOP
    BEGIN
    SET LOCAL lock_timeout = '5s';
      EXECUTE 'REVOKE SELECT (webhook_url) ON public.google_chat_webhooks FROM authenticated, anon';
    EXIT;
    EXCEPTION
      WHEN deadlock_detected OR lock_not_available THEN
        _ddl_try := _ddl_try + 1;
